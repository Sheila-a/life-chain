...
const Grid = ({ items, count = 6, className }) => {
  // set items length by count
  items = items || Array.from({ length: count });
  return (
    <div
      className={`grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-5 ${className}`}
    >
      {items.map((item, i) => (
        <div className="rounded-md bg-white   h-full p-6 shadow-base" key={i}>
          <div className="animate-pulse">
            ...
            <div className="grid grid-cols-2 gap-5 mt-6">
              ...
              <div>
                <span className="h-[18px] bg-[#C4C4C4]  w-[130px] inline-block rounded-full"></span>
              </div>
            </div>
          </div>
        </div>
